...
const initialState: FormState = {
  brandName: "",
  brandObjective: "",
  targetPlatform: [], // Multiple: Instagram, Facebook, LinkedIn, WhatsApp, Google
  industry: "",
  industryType: [],
  offering: "",
  audienceSegment: "",
...
export const AIPage: React.FC = () => {
  ...
  // Target Platforms multiselect
  const [platformOpen, setPlatformOpen] = useState(false);
  const platformRef = useRef<HTMLDivElement | null>(null);
  const platformOptions: (FormState["targetPlatform"][number])[] = [
    "Instagram",
    "Facebook",
    "LinkedIn",
    "WhatsApp",
  ...
  const addPlatform = (val: FormState["targetPlatform"][number]) => {
    setForm((s) => {
      if (s.targetPlatform.includes(val)) return s;
      return { ...s, targetPlatform: [...s.targetPlatform, val] };
    });
  };
  const removePlatform = (val: FormState["targetPlatform"][number]) => {
    setForm((s) => ({
      ...s,
      targetPlatform: s.targetPlatform.filter((x) => x !== val),
    }));
  };
  ...
  const recommend = (f: FormState): Recommendations => {
    ...
    const preferredPlatforms = Array.from(
      new Set(
        (
          Array.isArray(f.targetPlatform)
            ? f.targetPlatform
            : String((f as any).targetPlatform || "").split(/[,|\n]+|\s{2,}/)
        )
          .map((p) => normalizePlatform(String(p)))
          .filter(Boolean)
      )
    );
    ...
    ourUnderstanding.push(
      ...
      `Target platform focus: ${Array.isArray(f.targetPlatform) ? (f.targetPlatform.join(", ") || "-") : (String((f as any).targetPlatform || "-") )}`
    );
    ...
    <div className="bg-white dark:bg-[#1e1e1e] min-h-screen flex flex-col transition-colors duration-300 pb-12 [padding-bottom:env(safe-area-inset-bottom)]">
      ...
      <main className="w-full px-3 sm:px-6 md:px-10 lg:px-16">
        <div className="max-w-2xl sm:max-w-[1752px] mx-auto">
          ...
          <div className="flex flex-col" ref={platformRef}>
            <label className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-200">Target Platforms</label>
            <button
              type="button"
            ...
            >
              <span className="text-left flex-1 min-w-0">
                {form.targetPlatform.length ? (
                  <span className="flex flex-wrap gap-1.5 items-center">
                    {form.targetPlatform.slice(0, 2).map((t) => (
                      <span key={t} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-100 text-orange-800 dark:bg-[#2a2a2a] dark:text-orange-300 text-xs font-medium">
                        {t}
                        <button
                          type="button"
                          className="ml-1 text-orange-700 dark:text-orange-300"
                          onClick={(e) => { e.stopPropagation(); removePlatform(t); }}
                          aria-label={`Remove ${t}`}
                        >
                          ×
                        </button>
                    ...
                    {form.targetPlatform.length > 2 && (
                      <span className="text-xs text-gray-600 dark:text-gray-300">+{form.targetPlatform.length - 2} more</span>
                    )}
                  </span>
                ) : (
              ...
              <svg className={`w-4 h-4 shrink-0 transition-transform ${platformOpen ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path fillRule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z" clipRule="evenodd" />
              </svg>
            </button>
            {platformOpen && (
              <div className="mt-1 max-h-56 overflow-auto rounded-md border border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1b1b1b] shadow">
                {platformOptions.map((opt) => {
                  const selected = form.targetPlatform.includes(opt);
                  return (
                    <button
                      key={opt}
                      type="button"
                      onClick={() => (selected ? removePlatform(opt) : addPlatform(opt))}
                      className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between hover:bg-orange-50/70 dark:hover:bg-[#2a2a2a] ${selected ? "bg-orange-50/70 dark:bg-[#232323]" : ""}`}
                    >
                      <span>{opt}</span>
                      {selected && <span className="text-[#ffa500] font-semibold">✓</span>}
          ...
          {recos && (
            <section className="mt-12 grid grid-cols-1 gap-7 md:gap-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-7 md:gap-8">
                <Card title="Selected Platforms" items={Array.isArray(form.targetPlatform) ? form.targetPlatform : (form.targetPlatform ? [String((form as any).targetPlatform)] : [])} />
                {recos.platformsRecommended && <Card title="We recommend" items={recos.platformsRecommended} />}
              </div>
